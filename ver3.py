			...
			for k in C:
				if id[i] < id[j] and id[j] < id[k]:
					if LP[i][j] * LP[j][k] < LP[i][k]:
						...
						#order.addSell(i+k, 200, LP[i][k])
						'''
						if k == 'USD':
							profit = LP[i][k]-LP[i][j]*LP[j][k]
						elif id['USD'] < id[k]:
							profit = (LP[i][k]-LP[i][j]*LP[j][k])/LP['USD'][k]
						else:
							profit = (LP[i][k]-LP[i][j]*LP[j][k])/LP[k]['USD']
						
						best.append([profit, [i, j, k]])
						'''
