			...
			for k in C:
				if id[i] < id[j] and id[j] < id[k]:
					if LP[i][j] * LP[j][k] < LP[i][k]:
						...
						order.addBuy(j+k, int(LP[i][j]*100), LP[j][k])
						order.addBuy(i+j, 100, LP[i][j])
						order.addSell(i+k, 100, LP[i][k])
						print 'buying', int(LP[i][j]*100), 'of', j+k, 'at', LP[j][k]
						print 'buying', 100, 'of', i+j, 'at', LP[i][j]
						print 'selling', 100, 'of', i+k, 'at', LP[i][k]
						#print 'trade'
